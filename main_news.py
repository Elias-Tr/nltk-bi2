...
def preprocessing():
    ...
    f = open('tesla_news.txt', 'r', encoding='utf8')
    raw = f.read()
...
def get_cleared_text(text):
    ...
    #this will take some time if commented in!
    #cleared = spellchecker(cleared)
    #
    #enter the file that you want to write back into
...
#this method doesnt use the nltk dispersion plotter - use the other one
...
#unusued but can give the frequency of words with attached conditions - in this case the length - condition refers to
...
#helper method that returns only the specified Part of Speech values within an Part of speech ta
...
#somewhat deprecated
#plots a frequency distribution within nltk // this one has the option to only evaluate a certain pos
...
def frequency_dist_dict(cleared_list):
    ...
    only_nouns = return_specified_pos("NN", cleared_list)
    only_verbs = return_specified_pos("VB", cleared_list)

    #give this only_nouns or only_verbs if you want a distribution of those
